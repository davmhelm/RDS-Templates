...
param(
	...
	[Parameter(mandatory = $false)]
	[switch]$UseARMAPI = $true,

	# Useful in certain situations like Azure Lighthouse where RBAC role assignments can't be collected/viewed except in certain special cases
	# Use at your own risk
	[Parameter(Mandatory = $false)]
	[switch]$SkipRoleAssignmentCheck,
	...
	[Parameter(mandatory = $false)]
	[string]$LogAnalyticsEnvironmentName = 'AzureCloud',

	# The parameters for runas connections and corresponding environments have been modified
	# This script should support any combination of ARM WVD Host Pools belonging to subscriptions in Azure Commercial or Gov
	# Rather than building for the corner case that both subscriptions are in the same cloud, 
	# each should have its own connection (which includes a subscription ID as part of the required parameters)
	[Parameter(mandatory = $false)]
	[string]$HostPoolConnectionAssetName = 'AzureWvdHostPoolRunAsConnection',

	# Modify for WVD Host Pool resource running in different cloud (AzureUSGovernment)
	[Parameter(mandatory = $false)]
	[string]$HostPoolConnectionEnvironmentName = 'AzureCloud',

	# Modify for WVD Host Pool VM Instance resource(s) running in different cloud (AzureUSGovernment)
	[Parameter(mandatory = $false)]
	[string]$VMInstanceConnectionAssetName = 'AzureWvdVmInstanceRunAsConnection',

	# Modify for WVD Host Pool VM Instance resource(s) running in different cloud (AzureUSGovernment)
	[Parameter(mandatory = $false)]
	[string]$VMInstanceConnectionEnvironmentName = 'AzureCloud',
	...
	[Parameter(mandatory = $false)]
	[string]$ArtifactsURI = 'https://raw.githubusercontent.com/davmhelm/RDS-Templates/master/wvd-templates/wvd-scaling-script/multi_sub'
)
...
# //todo refactor -- remove RDSAPI support entirely or rewrite RDS API basicScale script to support multiSub
if ($UseRDSAPI) {
	throw "RDS API not supported for MultiSub autoscaler at this time. Set parameter `$UseARMAPI = `$true and try again."
}
...
[PSCustomObject]$RequestBody = @{
	...
	"LogAnalyticsEnvironmentName"         = $LogAnalyticsEnvironmentName
	"HostPoolConnectionAssetName"         = $HostPoolConnectionAssetName
	"VMInstanceConnectionAssetName"       = $VMInstanceConnectionAssetName
	"HostPoolConnectionEnvironmentName"   = $HostPoolConnectionEnvironmentName
	"VMInstanceConnectionEnvironmentName" = $VMInstanceConnectionEnvironmentName
	...
	#"EnvironmentName"                    = $AzContext.Environment.Name  # Note: not used in this scenario because Host Pool and VM Instances can be in different environments
	"UseARMAPI"                           = $UseARMAPI
