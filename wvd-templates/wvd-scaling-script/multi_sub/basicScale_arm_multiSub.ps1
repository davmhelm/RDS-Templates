...
try {
	[version]$Version = '0.1.38_multisub_alpha'
	...
	[string]$LogAnalyticsEnvironmentName = Get-PSObjectPropVal -Obj $RqtParams -Key 'LogAnalyticsEnvironmentName'
	[string]$HostPoolConnectionAssetName = Get-PSObjectPropVal -Obj $RqtParams -Key 'HostPoolConnectionAssetName'
	[string]$HostPoolConnectionEnvironmentName = Get-PSObjectPropVal -Obj $RqtParams -Key 'HostPoolConnectionEnvironmentName'
	[string]$VMInstanceConnectionAssetName = Get-PSObjectPropVal -Obj $RqtParams -Key 'VMInstanceConnectionAssetName'
	[string]$VMInstanceConnectionEnvironmentName = Get-PSObjectPropVal -Obj $RqtParams -Key 'VMInstanceConnectionEnvironmentName'
	...
	if ([string]::IsNullOrWhiteSpace($HostPoolConnectionAssetName)) {
		$HostPoolConnectionAssetName = 'AzureWvdHostPoolRunAsConnection'
	}
	if ([string]::IsNullOrWhiteSpace($HostPoolConnectionEnvironmentName)) {
		$HostPoolConnectionEnvironmentName = 'AzureCloud'
	}

	if ([string]::IsNullOrWhiteSpace($VMInstanceConnectionAssetName)) {
		$VMInstanceConnectionAssetName = 'AzureWvdVmInstanceRunAsConnection'
	}
	if ([string]::IsNullOrWhiteSpace($VMInstanceConnectionEnvironmentName)) {
		$VMInstanceConnectionEnvironmentName = 'AzureCloud'
	}
	...
	function Write-Log {
		...
		try {
			...
			$PostResult = Send-OMSAPIIngestionFile -customerId $LogAnalyticsWorkspaceId -sharedKey $LogAnalyticsPrimaryKey -Body $json_body -logType 'WVDTenantScale_CL' -TimeStampField 'TimeStamp' -EnvironmentName $LogAnalyticsEnvironmentName
	...
	if (!$SkipAuth) {
		...
		Write-Log "Get auto connection from asset: '$HostPoolConnectionAssetName'"
		$HostPoolConnectionAsset = Get-AutomationConnection -Name $HostPoolConnectionAssetName

		# Collect the credentials from Azure Automation Account Assets
		Write-Log "Get auto connection from asset: '$VMInstanceConnectionAssetName'"
		$VMInstanceConnectionAsset = Get-AutomationConnection -Name $VMInstanceConnectionAssetName
		
		# Azure auth
		$AzHostPoolContext = $null
		$AzVMInstanceContext = $null
		
		# WVD HostPool Connection first
		$AzAuth = $null
		try {
			$AzAuth = Connect-AzAccount -ApplicationId $HostPoolConnectionAsset.ApplicationId -CertificateThumbprint $HostPoolConnectionAsset.CertificateThumbprint -TenantId $ConnectionAsset.TenantId -SubscriptionId $ConnectionAsset.SubscriptionId -EnvironmentName $HostPoolConnectionEnvironmentName -ServicePrincipal
			if (!$AzAuth -or !$AzAuth.Context) {
				throw $AzAuth
			}
			$AzHostPoolContext = $AzAuth.Context
		}
		catch {
			throw [System.Exception]::new('Failed to authenticate Azure with application ID, tenant ID, subscription ID', $PSItem.Exception)
		}
		Write-Log "Successfully authenticated with Azure using service principal: $($AzHostPoolContext | Format-List -Force | Out-String)"

		# VM Instance Connection next
		$AzAuth = $null
		try {
			$AzAuth = Connect-AzAccount -ApplicationId $VMInstanceConnectionAsset.ApplicationId -CertificateThumbprint $VMInstanceConnectionAsset.CertificateThumbprint -TenantId $ConnectionAsset.TenantId -SubscriptionId $ConnectionAsset.SubscriptionId -EnvironmentName $VmInstanceConnectionEnvironmentName -ServicePrincipal
			if (!$AzAuth -or !$AzAuth.Context) {
				throw $AzAuth
			}
			$AzVMInstanceContext = $AzAuth.Context
		}
		catch {
			throw [System.Exception]::new('Failed to authenticate Azure with application ID, tenant ID, subscription ID', $PSItem.Exception)
		}
		Write-Log "Successfully authenticated with Azure using service principal: $($AzVMInstanceContext | Format-List -Force | Out-String)"

		# Set Azure context with subscription, tenant
		# These need to be available as we interact with different components (Host Pool object or VM Instances)
		if (AzHostPoolContext.Tenant.Id -ine $HostPoolConnectionAsset.TenantId -or $AzHostPoolContext.Subscription.Id -ine $HostPoolConnectionAsset.SubscriptionId) {
			if ($PSCmdlet.ShouldProcess((@($ConnectionAsset.TenantId, $ConnectionAsset.SubscriptionId) -join ', '), 'Set Azure context with tenant ID, subscription ID')) {
				try {
					$AzHostPoolContext = Get-AzContext -TenantId $HostPoolConnectionAsset.TenantId -SubscriptionId $HostPoolConnectionAsset.SubscriptionId
					if (!$AzHostPoolContext -or $AzHostPoolContext.Tenant.Id -ine $HostPoolConnectionAsset.TenantId -or $AzHostPoolContext.Subscription.Id -ine $HostPoolConnectionAsset.SubscriptionId) {
						throw $AzHostPoolContext
					}
				}
				catch {
					throw [System.Exception]::new('Failed to get Azure context with tenant ID, subscription ID', $PSItem.Exception)
				}
				Write-Log "Successfully got the Azure context with the tenant ID, subscription ID: $($AzHostPoolContext | Format-List -Force | Out-String)"
			}
		}

		if ($AzVMInstanceContext.Tenant.Id -ine $VMInstanceConnectionAsset.TenantId -or $AzVMInstanceContext.Subscription.Id -ine $VMInstanceConnectionAsset.SubscriptionId) {
			if ($PSCmdlet.ShouldProcess((@($VMInstanceConnectionAsset.TenantId, $VMInstanceConnectionAsset.SubscriptionId) -join ', '), 'Set Azure context with tenant ID, subscription ID')) {
				try {
					$AzVMInstanceContext = Get-AzContext -TenantId $VMInstanceConnectionAsset.TenantId -SubscriptionId $VMInstanceConnectionAsset.SubscriptionId
					if (!$AzVMInstanceContext -or $AzVMInstanceContext.Tenant.Id -ine $VMInstanceConnectionAsset.TenantId -or $AzVMInstanceContext.Subscription.Id -ine $VMInstanceConnectionAsset.SubscriptionId) {
						throw $AzVMInstanceContext
					}
				}
				catch {
					throw [System.Exception]::new('Failed to get Azure context with tenant ID, subscription ID', $PSItem.Exception)
				}
				Write-Log "Successfully got the Azure context with the tenant ID, subscription ID: $($AzVMInstanceContext | Format-List -Force | Out-String)"
			}
		}
	}

	# ESTABLISH DEFAULT AZ POWERSHELL CONTEXT: WVD HOST POOL SUB/TENANT
	try {
		$AzHostPoolContext | Select-AzContext
	} catch {
		throw [System.Exception]::new('Failed to set default Azure context with tenant ID, subscription ID', $PSItem.Exception)
	}
	#endregion
	...
	# //todo: consider building VM list additively rather than reductively; script is designed to fail if VM hosts can't be found, and we have the exact ResourceID(s) of the underlying VM(s) from the $SessionHosts object
	foreach ($VMInstance in (Get-AzVM -Status -DefaultProfile $AzVMInstanceContext)) {
		...
		if (!$VMSizeCores.ContainsKey($VMInstance.HardwareProfile.VmSize)) {
			...
			foreach ($VMSize in (Get-AzVMSize -Location $VMInstance.Location -DefaultProfile $AzVMInstanceContext)) {
	...
	if ($Ops.nVMsToStart -or $Ops.nCoresToStart) {
		...
		foreach ($VM in $VMs.Values) {
			...
			if ($PSCmdlet.ShouldProcess($SessionHostName, 'Start session host as a background job')) {
				...
				$StartVMjobs += ($VM.Instance | Start-AzVM -AsJob -DefaultProfile $AzVMInstanceContext)
			}
	...
	if ($VMsToStopAfterLogOffTimeOut) {
		...
		foreach ($VM in $VMsToStopAfterLogOffTimeOut) {
			...
			if ($PSCmdlet.ShouldProcess($SessionHostName, 'Stop session host as a background job')) {
				...
				$StopVMjobs += ($VM.StopJob = $VM.Instance | Stop-AzVM -Force -AsJob -DefaultProfile $AzVMInstanceContext)
				$VMsToStop.Add($SessionHostName, $VM)
			}
		}
	}
